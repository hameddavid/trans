  ...
  <div class="max-w-lg mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <button type="submit" :disabled="loading || mismatch" class="w-full flex justify-center py-2.5 px-4 rounded-lg text-white bg-run-dark hover:bg-run-dark/90 font-medium disabled:opacity-50">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <dl class="space-y-3">
        ...
        <div class="flex justify-between">
          ...
          <dd>
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :class="user?.role === '300' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'">
              {{ user?.role === '300' ? 'Approver' : 'Recommender' }}
            </span>
          </dd>
        </div>
      </dl>
...
async function handleSubmit() {
  ...
  errors.value = {}
  try {
    await adminAuthStore.resetPassword(form)
