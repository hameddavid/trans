<template>
  <div class="space-y-6">
    <h1 class="text-2xl font-bold text-gray-900">Approved Student Applications</h1>
    ...
    <div v-else>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-if="filteredApplications.length === 0">
              <td colspan="5" class="px-4 py-5 text-center text-xs text-gray-500">
                <DocumentIcon class="mx-auto h-12 w-12 text-gray-400 mb-2" />
                No approved student applications found
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <AppPagination
        ...
        @page-change="(p) => adminApplicationsStore.fetchApprovedStudent(p)"
        @per-page-change="(pp) => adminApplicationsStore.fetchApprovedStudent(1, pp)"
      />
    </div>
...
const pag = computed(() => adminApplicationsStore.pagination.approvedStudent);
...
const filteredApplications = computed(() => {
  ...
  if (!q) return adminApplicationsStore.approvedStudent;
  return adminApplicationsStore.approvedStudent.filter(
    (app) =>
...
async function viewTranscript(app) {
  ...
  try {
    const { data } = await adminApi.getTranscriptHtml('student', app.id);
...
async function handleRegenerate(app) {
  ...
  adminApplicationsStore.fetchApprovedStudent(pag.value.currentPage);
}

onMounted(() => {
  adminApplicationsStore.fetchApprovedStudent();
});
</script>
