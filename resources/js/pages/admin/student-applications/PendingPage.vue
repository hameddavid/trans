<template>
  <div class="space-y-6">
    <h1 class="text-2xl font-bold text-gray-900">Pending Student Applications</h1>
    ...
    <div v-else>
      ...
      <AppPagination
        ...
        @page-change="(p) => adminApplicationsStore.fetchPendingStudent(p)"
        @per-page-change="(pp) => adminApplicationsStore.fetchPendingStudent(1, pp)"
      />
    </div>
...
const pag = computed(() => adminApplicationsStore.pagination.pendingStudent);
...
const filteredApplications = computed(() => {
  ...
  if (!q) return adminApplicationsStore.pendingStudent;
  return adminApplicationsStore.pendingStudent.filter(
    (app) =>
...
async function handleRecommend(app) {
  ...
  adminApplicationsStore.fetchPendingStudent(pag.value.currentPage);
}
...
onMounted(() => {
  adminApplicationsStore.fetchPendingStudent();
});
</script>
