<template>
  <div class="space-y-6">
    <h1 class="text-2xl font-bold text-gray-900">Recommended Student Applications</h1>
    ...
    <div v-else>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-if="filteredApplications.length === 0">
              <td colspan="6" class="px-4 py-5 text-center text-xs text-gray-500">
                ...
                No recommended student applications found
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <AppPagination
        ...
        @page-change="(p) => adminApplicationsStore.fetchRecommendedStudent(p)"
        @per-page-change="(pp) => adminApplicationsStore.fetchRecommendedStudent(1, pp)"
      />
    </div>
...
const pag = computed(() => adminApplicationsStore.pagination.recommendedStudent);
...
const filteredApplications = computed(() => {
  ...
  if (!q) return adminApplicationsStore.recommendedStudent;
  return adminApplicationsStore.recommendedStudent.filter(
    (app) =>
...
async function handleApprove(app) {
  ...
  adminApplicationsStore.fetchRecommendedStudent(pag.value.currentPage);
}
...
onMounted(() => {
  adminApplicationsStore.fetchRecommendedStudent();
});
</script>
